...
execute_jar() {
    ...
    chmod 755 $log_dir
    # IP 주소 가져오기
    local ip=$(hostname -I | awk '{print $1}' | tr -d ' ')
    # 날짜 형식 지정
    local date=$(date +%Y%m%d_%H%M%S)
    # 로그 파일 이름 생성
    local log_file="${ip}_${port}_${date}_nohup.log"
    echo "로그 파일 이름: $log_file"
    echo "실행 기록에 저장..."
    ...
    if [ "$run_in_background" = true ]; then
        ...
        # 포그라운드에서 실행
        $command 2>&1 | tee "$log_dir/$log_file"
    fi 
}
...
create_log_dir() {
    ...
    local log_dir="$(pwd)/logs/$date_dir/$jar_name"
    mkdir -p $log_dir
    chmod 755 $log_dir
    echo $log_dir
}
...
while true; do
    ...
    case $choice in
        ...
        5)
            ...
            ;;
        6)
         #ex) java -jar department.jar --server.port=9998 --spring.datasource.jdbc-url="jdbc:oracle:thin:@172.30.1.39:1521/TESTDB"
         #Java 옵션: -Xmx1g -Xms512m -Dspring.config.location=file:/path/to/config.yml
         #JAR 옵션: --server.port=9090 --spring.profiles.active=prod            
            read -p "JAR 파일 실행 옵션을 입력하세요: " jar_options
            port=$(echo $jar_options | grep -oP '(?<=--server.port=)\d+' || echo "8080")
            read -p "백그라운드에서 실행하시겠습니까? (y/n): " bg_choice
            if [[ $bg_choice == "y" ]]; then
                execute_jar "java -jar $selected_file $jar_options" $log_dir $port true
            else
                execute_jar "java -jar $selected_file $jar_options" $log_dir $port false
            fi
            ;;
        *)
            echo "잘못된 선택입니다. 다시 시도해주세요."
