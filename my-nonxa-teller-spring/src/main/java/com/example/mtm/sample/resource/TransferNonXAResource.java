...
public class TransferNonXAResource {
	...
	public ResponseEntity<?> testWithoutTran(@RequestBody
	Transfer transferDetails) throws Exception {
	...
	public ResponseEntity<?> transfer(@RequestBody
	Transfer transferDetails) throws Exception {
		...
		if (!withdrawResponse.getStatusCode().is2xxSuccessful()) {
			...
			throw new TransferFailedException(
				String.format("Withdraw failed: %s Reason: %s", transferDetails, withdrawResponse.getBody()));
		}
